      ...
      <div className="flex min-w-0 w-full items-center gap-2 text-sm">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1">
          {isSubmitting || interruptPending ? <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><Loader2 className="size-3.5 animate-spin" />{t.chat.queueSubmitting}</span> : isSteering || steerPending ? <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><Loader2 className="size-3.5 animate-spin" />{t.chat.queueDelivering}</span> : <>
            <Button type="button" variant="outline" size="sm" disabled={pendingAction !== null} title={t.chat.queueEditDescription} onClick={() => void runAction("edit", onEdit)}>{pendingAction === "edit" ? <Loader2 className="size-3.5 animate-spin" /> : <Pencil className="size-3.5" />}{t.chat.queueEdit}</Button>
            <Button type="button" variant="outline" size="sm" disabled={pendingAction !== null} title={t.chat.queueSteerDescription} onClick={() => void runAction("steer", onSteer)}>{steerPending ? <Loader2 className="size-3.5 animate-spin" /> : <Send className="size-3.5" />}{t.chat.queueSteer}</Button>
            <Button type="button" variant="outline" size="sm" disabled={pendingAction !== null} title={t.chat.queueInterruptDescription} onClick={() => void runAction("interrupt", onInterrupt)}>{interruptPending ? <Loader2 className="size-3.5 animate-spin" /> : <Square className="size-3.5" />}{t.chat.queueInterrupt}</Button>
