...
interface PromptCheckpointContextValue {
  checkpointByMessageId: Map<string, ChatCheckpointSummary>
  onRestoreCheckpoint?: (
    checkpointId: string,
...
const PromptCheckpointContext = React.createContext<PromptCheckpointContextValue>({
...
export function PromptCheckpointProvider({
  ...
  onRestoreCheckpoint,
  children,
}: PromptCheckpointContextValue & { children: React.ReactNode }) {
  const value = useMemo(() => ({
    checkpointByMessageId,
    onRestoreCheckpoint,
  }), [checkpointByMessageId, onRestoreCheckpoint])

  return (
    <PromptCheckpointContext.Provider value={value}>
      {children}
    </PromptCheckpointContext.Provider>
  )
}
...
function sameMessage(left: HydratedTranscriptMessage, right: HydratedTranscriptMessage) {
  ...
  switch (left.kind) {
    ...
    case "rate_limit_updated":
      return right.kind === "rate_limit_updated" && JSON.stringify(left.rateLimits) === JSON.stringify(right.rateLimits)
...
interface TranscriptSingleRowProps {
  ...
  hideResult: boolean
  isFinalStatus: boolean
  promptCheckpoint?: ChatCheckpointSummary | null
  onRestoreCheckpoint?: (
    checkpointId: string,
    mode: CheckpointRestoreMode,
    promptContent: string
...
const TranscriptSingleRow = memo(function TranscriptSingleRow({
  ...
  if (message.kind === "user_prompt") {
    const resolvedPromptCheckpoint = promptCheckpoint ?? promptCheckpointContext.checkpointByMessageId.get(message.id) ?? null
    const resolvedRestoreCheckpoint = onRestoreCheckpoint ?? promptCheckpointContext.onRestoreCheckpoint
    rendered = (
      <UserMessage
        key={message.id}
        ...
        checkpoint={resolvedPromptCheckpoint}
        onRestoreCheckpoint={resolvedRestoreCheckpoint}
      />
    )
  } else {
    switch (message.kind) {
...
interface AbolqasemTranscriptProps {
  messages: HydratedTranscriptMessage[]
  isLoading: boolean
  localPath?: string
  latestToolIds: Record<string, string | null>
...
function AbolqasemTranscriptImpl({
  messages,
  isLoading,
  localPath,
  latestToolIds,
...
export const AbolqasemTranscript = memo(AbolqasemTranscriptImpl)
