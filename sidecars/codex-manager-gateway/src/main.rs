...
async fn health(State(state): State<AppState>) -> Response<Body> {
    json_response(
        ...
        &serde_json::json!({
            ...
            "configuredAccounts": account_candidates(&state).len(),
            "loopback": true,
        }),
    )
}
...
async fn gateway(State(state): State<AppState>, request: Request<Body>) -> Response<Body> {
    ...
    let candidates = account_candidates(&state)
        .into_iter()
        .filter(|candidate| !cooling_down(&state, &candidate.name))
        .collect::<Vec<_>>();
    ...
    let selected = if let Some(key) = conversation.as_deref() {
        let bindings = state.bindings.lock().expect("binding lock");
        bindings
            .account(key)
            .and_then(|bound| candidates.iter().find(|candidate| candidate.name == bound))
            .cloned()
    } else {
        None
    };
    ...
    if let Some(candidate) = selected.clone() {
    ...
    let upstream = 'accounts: loop {
        for candidate in &candidates_to_try {
            ...
            // Both attempts occur before any response body is exposed. A
            // started SSE stream is never replayed against another account.
            ...
            if matches!(status_code, 401 | 403 | 429) {
                record_cooldown(&state, &candidate.name, status_code);
                continue;
            }
            if let Some(key) = &conversation {
                let reason = if selected
                    .as_ref()
                    .is_some_and(|bound| bound.name == candidate.name)
                {
                    "preserved"
                ...
                if state
                    ...
                    .lock()
                    .expect("binding lock")
                    .bind(key.clone(), candidate.name.clone(), reason)
                    .is_err()
                {
                    return response(
                        StatusCode::INTERNAL_SERVER_ERROR,
                        "could not persist account binding\n",
                    );
                }
            }
...
fn cooling_down(state: &AppState, account: &str) -> bool {
    let now = std::time::Instant::now();
    let mut cooldowns = state.cooldowns.lock().expect("cooldown lock");
    cooldowns.retain(|_, until| *until > now);
    cooldowns.get(account).is_some_and(|until| *until > now)
}

fn record_cooldown(state: &AppState, account: &str, status: u16) {
    let delay = match status {
        429 => Duration::from_secs(60),
        401 | 403 => Duration::from_secs(5 * 60),
        _ => return,
    };
    state
        .cooldowns
        ...
        .insert(account.to_owned(), std::time::Instant::now() + delay);
}
...
async fn main() {
    ...
    let state = AppState {
        ...
        bindings: Arc::new(Mutex::new(BindingStore::load(
            manager_home.join("bindings.json"),
        ))),
...
"#;
    ...
    fn cooldown_excludes_account_until_expiry() {
        let temporary =
            std::env::temp_dir().join(format!("cm-cooldown-test-{}", std::process::id()));
        let state = super::AppState {
            accounts_dir: temporary.join("accounts"),
            status_dir: temporary.join("status"),
            models_cache: temporary.join("models.json"),
            upstream_base: "https://example.invalid".into(),
            api_key: "key".into(),
            client: reqwest::Client::new(),
            cursor: std::sync::Arc::new(std::sync::Mutex::new(0)),
            bindings: std::sync::Arc::new(std::sync::Mutex::new(super::BindingStore::load(
                temporary.join("bindings.json"),
            ))),
            cooldowns: std::sync::Arc::new(std::sync::Mutex::new(std::collections::HashMap::new())),
        };
        assert!(!super::cooling_down(&state, "account"));
        super::record_cooldown(&state, "account", 429);
        assert!(super::cooling_down(&state, "account"));
    }
}
