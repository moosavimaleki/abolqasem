# Abolqasem Feature Inventory

This document lists the main product, UI, backend, and agent features observed in
`/home/h-mousavi/Projects/Hamed/abolqasem`. It is intended as a reference for
...
- Local data directory under `~/.abolqasem/data`.
...
- Abolqasem owns the chat runtime; current `ai-agent-manager` mostly observes existing transcripts.
- Abolqasem persists domain events; current `ai-agent-manager` persists session metadata.
- Abolqasem streams live provider events; current `ai-agent-manager` mostly reloads parsed transcript data.
- Abolqasem has an agent coordinator; current `ai-agent-manager` has a thin Codex turn endpoint.
- Abolqasem has provider-specific model/options abstractions; current `ai-agent-manager` has simpler model inputs.
- Abolqasem has tool approval loops; current `ai-agent-manager` rejects or cannot handle most tool requests.
- Abolqasem has project/git/terminal workflows; current `ai-agent-manager` is primarily session/message viewing.
