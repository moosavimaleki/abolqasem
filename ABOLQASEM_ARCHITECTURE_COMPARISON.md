# Abolqasem Architecture Compared With ai-agent-manager

این سند تفاوت معماری `Abolqasem` با وضعیت فعلی `ai-agent-manager` را توضیح می‌دهد، مخصوصاً در بخش server architecture، مدل پیام‌ها، commandها، eventها و جریان رفت‌وبرگشت داده بین UI، backend و agentها.
...
`Abolqasem` یک agent workbench است:
...
### فایل‌های مهم Abolqasem

- [src/shared/protocol.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/shared/protocol.ts)
- [src/shared/types.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/shared/types.ts)
- [src/server/events.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/server/events.ts)
- [src/server/event-store.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/server/event-store.ts)
- [src/server/read-models.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/server/read-models.ts)
- [src/server/ws-router.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/server/ws-router.ts)
- [src/server/agent.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/server/agent.ts)
- [src/server/codex-app-server.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/server/codex-app-server.ts)
- [src/server/provider-catalog.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/server/provider-catalog.ts)
- [src/server/diff-store.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/server/diff-store.ts)
- [src/server/terminal-manager.ts](/home/h-mousavi/Projects/Hamed/abolqasem/src/server/terminal-manager.ts)

## مدل داده در Abolqasem

Abolqasem به جای یک `state.json` ساده، چند event stream دارد:
...
## Read Model در Abolqasem

Abolqasem eventهای خام را مستقیم به UI نمی‌دهد. اول از آن‌ها read model می‌سازد:
...
## پروتکل پیام‌ها در Abolqasem

Abolqasem به جای endpointهای REST پراکنده، `ClientCommand` دارد. UI از طریق WebSocket command می‌فرستد.
...
در Abolqasem ارسال پیام یک command غنی است، نه فقط `message + cwd`:
...
Abolqasem فقط پاسخ request/response ساده ندارد. UI به topicها subscribe می‌شود و server snapshot یا event می‌فرستد.
...
- Abolqasem app-server را برای هر request نمی‌کشد و نابود نمی‌کند.
- Abolqasem raw eventها را می‌فهمد.
- Abolqasem tool request را به UI می‌آورد.
- Abolqasem approval را از UI برمی‌گرداند.
- Abolqasem plan mode را به collaboration mode وصل می‌کند.
- Abolqasem status زنده دارد.

## AgentCoordinator چه کاری می‌کند؟

در Abolqasem، `AgentCoordinator` مغز runtime است.
...
در Abolqasem provider فقط یک string نیست. هر provider این‌ها را دارد:
...
Abolqasem می‌تواند وسط اجرای agent این چرخه را اجرا کند:
...
در Abolqasem git بخشی از محصول است، نه یک feature جانبی.
...
Abolqasem terminal را از طریق WebSocket کنترل می‌کند:
...
## معماری پیشنهادی برای نزدیک شدن به Abolqasem
...
Abolqasem از اول برای کنترل agent طراحی شده است.
