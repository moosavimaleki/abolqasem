# Abolqasem Parity Port To Go Backend

این سند task specification برای port کردن Abolqasem روی پروژه `ai-agent-manager` است.

هدف این سند ساخت محصول جدید یا الهام‌گرفتن آزاد از Abolqasem نیست. هدف این است که Abolqasem تا حد ممکن با همان ظاهر، همان رفتار، همان UX و همان feature set منتقل شود؛ فقط backend به Go تبدیل شود و frontend دو زبانه و RTL شود.

فرض‌ها:

- Frontend ری‌اکتی Abolqasem باید تا حد ممکن مستقیم منتقل شود.
- Frontend باید bilingual شود: `fa` و `en`.
- زبان فارسی باید RTL کامل داشته باشد.
- Backend باید Go باشد، نه Bun/TypeScript.
- feature parity با Abolqasem مهم‌تر از طراحی feature جدید است.
- deviation از Abolqasem فقط وقتی مجاز است که برای Go idiomatic بودن، امنیت local server یا RTL/i18n لازم باشد.
- Gemini، hook behavior اختصاصی فعلی و viewer legacy نباید وارد core Abolqasem-port شوند مگر برای migration/compatibility.

## قانون‌های Port

### Rule 1: Copy First, Redesign Last

- frontend componentها، layout، interaction و naming تا حد ممکن از Abolqasem کپی شوند.
- backend behavior باید از Abolqasem mirror شود، نه اینکه feature جدید طراحی شود.
- اگر در Abolqasem command یا snapshot خاصی هست، در Go همان shape حفظ شود مگر دلیل جدی وجود داشته باشد.

### Rule 2: Protocol Parity

Go backend باید envelopeهای Abolqasem را حفظ کند:
...
نباید یک envelope جدید مثل `{type, data}` اختراع شود مگر frontend Abolqasem هم به همان تغییر منتقل شود. اولویت با کمترین تغییر در frontend است.

### Rule 3: No Extra Product Scope

موارد زیر در core parity scope نیستند مگر اینکه Abolqasem خودش داشته باشد:

- Gemini send/control.
- mode جدید اختصاصی برای viewer/web-ui.
- auth جدید برای remote usage.
- product flow جدید که در Abolqasem نیست.
- abstractionهای بیش از حد عمومی برای آینده نامعلوم.

### Rule 4: Go Idiomatic, Abolqasem Compatible

- ساختار packageهای Go می‌تواند idiomatic باشد.
- JSON shape، command names، snapshot names و UI expectations باید Abolqasem-compatible بمانند.
- تست‌ها باید behavior Abolqasem را lock کنند.

## هدف معماری

معماری هدف با parity نسبت به Abolqasem:
...
### Task 0.1: تهیه parity map از Abolqasem `[done]`

شرح:

- تمام فایل‌های `src/shared`, `src/server`, `src/client` در Abolqasem map شوند.
- برای هر فایل Abolqasem مشخص شود:
  - `copy frontend`
  - `port to Go`
...
- هیچ فیچر Abolqasem بدون تصمیم explicit حذف نشود.
- هر defer باید دلیل و phase داشته باشد.

Output:

- [ABOLQASEM_PARITY_MAP.md](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/ABOLQASEM_PARITY_MAP.md)

### Task 0.1.1: Server module port map `[done]`

این map باید هنگام اجرا به checklist واقعی تبدیل شود. معادل‌های پیشنهادی Go فقط naming هستند؛ behavior باید از Abolqasem بیاید.

```text
Abolqasem server module                  Go target
---------------------------------------------------------------------------
...
- testهای متناظر Abolqasem تا حد ممکن به Go test تبدیل شوند.
...
- `ClientCommand`ها با Abolqasem برابر باشند.
- `SubscriptionTopic`ها با Abolqasem برابر باشند.
- `ServerSnapshot`ها با Abolqasem برابر باشند.
...
- visual/component hierarchy با Abolqasem یکی بماند.
- فقط i18n/RTL و backend connection تغییر کند.
- testهای frontend Abolqasem تا حد ممکن حفظ شوند.
...
- license پروژه Abolqasem و dependencyهای آن بررسی شود.
...
- [ABOLQASEM_LICENSE_REVIEW.md](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/ABOLQASEM_LICENSE_REVIEW.md)
...
- ساختار React Abolqasem به مسیر frontend جدید منتقل شود.
- تغییرات فقط برای اتصال به Go backend، branding، i18n و RTL باشد.
- CSS/spacing/component hierarchy تا حد ممکن حفظ شود.

Acceptance criteria:

- UI visually نزدیک به Abolqasem باشد.
- snapshot/screenshot baseline از صفحات Abolqasem برای مقایسه گرفته شود.
- هیچ layout جدیدی بدون دلیل اضافه نشود.

Output:

- Frontend Abolqasem copied to [web-react](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/web-react).
...
- اگر Abolqasem با Vite/Bun است، frontend build می‌تواند همان Vite را نگه دارد.
...
- structure کلی Abolqasem حفظ شده باشد.
...
### Task 1.2: انتقال کامل shell UI Abolqasem `[done]`

شرح:

- layout اصلی Abolqasem منتقل شود:
  - sidebar
  - chat page
  - chat navbar
  - composer dock
...
- [x] ظاهر باید با Abolqasem قابل مقایسه باشد، نه طراحی جدید.
...
- [AbolqasemSidebar.tsx](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/web-react/src/client/app/AbolqasemSidebar.tsx)
...
- Existing Abolqasem code/diff/terminal/path components remain copied and keep their own LTR-oriented rendering.
...
- shapeها باید با Abolqasem یکی بمانند.
...
- frontend Abolqasem socket client با حداقل تغییر به Go وصل شود.
...
- Abolqasem-compatible envelope DTOs, topic constants, command constants, snapshot/event/ack/error helpers.
...
- `/auth/status` added so copied Abolqasem UI can pass auth check.
...
Commandهای Abolqasem باید mirror شوند:
...
- Unimplemented commands return Abolqasem-compatible error envelopes with the original command ID.
...
- Abolqasem `v: 2` event version preserved.
- Abolqasem-style top-level event fields preserved; domain fields are not wrapped under `data`.
...
- Append/replay implemented for Abolqasem streams: `projects`, `chats`, `messages`, `queued-messages`, `turns`.
...
- `snapshot.json` shape follows Abolqasem `SnapshotFile`: `v`, `generatedAt`, `projects`, `chats`, `queuedMessages`.
- Compaction writes snapshot atomically with temp+rename, then truncates Abolqasem event streams.
- Startup path loads snapshot first and replays later JSONL events with Abolqasem event ordering priority.
...
- Sidebar data is derived from Abolqasem-compatible project/chat events.
...
- `DeriveChatSnapshot` mirrors Abolqasem `deriveChatSnapshot`.
...
- Transcript messages/history are passed through with Abolqasem `ChatHistorySnapshot` shape.
- Available providers include the Abolqasem server provider catalog for `claude` and `codex`.
...
- `DeriveLocalProjectsSnapshot` mirrors Abolqasem saved/discovered merge behavior.
...
- Machine snapshot uses Abolqasem `local` machine shape.

## Milestone 5: Provider Catalog

### Task 5.1: Port مستقیم ProviderCatalog Abolqasem `[done]`
...
- provider list با Abolqasem برابر باشد مگر در migration legacy.
...
- Abolqasem `claude` and `codex` provider catalog ported.
- Codex server model list matches Abolqasem hard-coded server catalog.
...
- option naming با Abolqasem compatible باشد.

Output:

- `NormalizeServerModel` mirrors Abolqasem alias/default behavior.
...
- `CodexServiceTierFromModelOptions` maps fast mode to Abolqasem `fast` service tier.
- `ResolveClaudeAPIModelID` mirrors Abolqasem `1m` context model suffix behavior.
...
- Abolqasem coordinator invariants بدون adapter واقعی provider پیاده شود.
...
- Active statuses include Abolqasem statuses plus cancelled support for Go-side state handling.
...
- Queue persistence is delegated to the Abolqasem-compatible store interface/event store.
...
- `StartSession` chooses `thread/fork`, `thread/resume`, or `thread/start` using Abolqasem order.
...
- `item/tool/requestUserInput` maps to Abolqasem `ask_user_question` tool call and returns Codex answer shape.
...
### Task 8.1: Port رفتار Claude adapter Abolqasem به Go `[done]`

اصل:

- هدف کپی رفتار Abolqasem است.
...
- Stream parser maps assistant/result JSON lines into Abolqasem transcript entries.
...
- Gemini وارد provider catalog Abolqasem-port نشود مگر Abolqasem upstream اضافه کند.

Output:

- Existing legacy Gemini parser/discovery/hook code remains untouched.
- Provider catalog test locks Abolqasem parity providers to exclude Gemini.
- Gemini remains available only through legacy viewer compatibility.

### Task 9.2: No Gemini Web UI control in Abolqasem parity scope `[done]`

Acceptance criteria:

- هیچ UI control جدید برای Gemini ساخته نشود.
- هیچ مدل/option اختصاصی Gemini در Abolqasem workspace اضافه نشود.
- اگر بعداً نیاز شد، task جدا خارج از parity scope تعریف شود.

Output:

- No Gemini provider entry was added to Abolqasem Web UI provider catalog.
- No Gemini model/options were added to Abolqasem workspace scope.
...
- Read models alias the shared transcript type so chat snapshots keep Abolqasem-compatible JSON.
...
- Legacy `state.SessionMeta` plus parser messages are converted into Abolqasem-compatible project/chat/transcript snapshots.
...
- [coordinator.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/workspace/agent/coordinator.go)
- [coordinator_test.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/workspace/agent/coordinator_test.go)
- Abolqasem frontend already includes `AskUserQuestionMessage`, `ExitPlanModeMessage`, and `chat.respondTool` command dispatch.
...
- Non-git directories return Abolqasem-compatible `status: "no_repo"` with empty files.
...
- [detect.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/workspace/gitservice/detect.go)
- [detect_test.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/workspace/gitservice/detect_test.go)
- Git snapshots now populate Abolqasem-compatible `files` with `added`, `modified`, `deleted`, and `renamed` entries.
...
- `commit_only` and `commit_and_push` return Abolqasem-compatible success/failure result shapes.
...
- Local and remote branch lists return Abolqasem-compatible branch rows.
- Checkout and create branch commands return Abolqasem-compatible action results.
...
### Task 12.5: GitHub publish workflow مطابق Abolqasem `[done]`
...
- response shape با Abolqasem برابر باشد.
...
- Repo availability uses `gh repo view` and returns Abolqasem-compatible `available/message`.
- Publish uses `gh repo create` plus a real branch push through the Git service.

### Task 12.6: Discard/ignore workflow مطابق Abolqasem `[done]`

Acceptance criteria:

- `chat.discardDiffFile` پیاده شود.
- `chat.ignoreDiffFile` پیاده شود.
- ignore file/folder behavior با UI Abolqasem سازگار باشد.
...
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- Existing copied Abolqasem React UI already includes xterm.js panes, terminal layout store, multi-terminal UI, and persisted layout state.
...
- Markdown, JSON/CSV/TSV, known code extensions, image/PDF, and generic binary MIME types are returned for Abolqasem previews.
...
- `settings.writeAppSettingsPatch` applies Abolqasem-compatible partial patches and returns the updated snapshot.
...
### Task 15.3: Keybindings مطابق Abolqasem ✅

Acceptance criteria:

- `settings.readKeybindings` پیاده شود. ✅
- `settings.writeKeybindings` پیاده شود. ✅
- default keybindings با Abolqasem برابر باشد. ✅
...
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- Keybindings are persisted in `keybindings.json` under the app state directory and are created from Abolqasem defaults when missing.
- Invalid or empty keybinding files fall back to Abolqasem defaults with a warning snapshot.
...
### Task 15.4: LLM provider settings مطابق Abolqasem ✅
...
- provider kindهای Abolqasem حفظ شوند: `openai`, `openrouter`, `custom`. ✅
...
- Provider normalization follows Abolqasem: `openai`, `openrouter`, and `custom`, with the same base URLs and default models.
...
- Validation sends an OpenAI-compatible `/responses` request when configuration is complete and returns Abolqasem-compatible config errors otherwise.

## Milestone 16: Frontend Integration With Go Protocol

### Task 16.1: اتصال Abolqasem socket client با کمترین تغییر ✅

شرح:

- TypeScript socket client موجود در Abolqasem حفظ شود.
- فقط URL، error handling لازم و type compatibility با Go تنظیم شود.
- command envelope همان Abolqasem بماند.
...
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- Abolqasem socket client continues to use `/ws` unchanged.
...
- `chat.respondTool` is wired to the Go coordinator and tool call/result transcript entries are recorded for Abolqasem approval cards.

## Milestone 17: Legacy Viewer Bridge

### Task 17.1: نگه داشتن viewer فعلی خارج از Abolqasem workspace ✅

Acceptance criteria:

- [x] viewer فعلی می‌تواند پشت route legacy بماند.
- [x] نباید UI اصلی Abolqasem-port را با behaviorهای hook-follow فعلی آلوده کند.
- [x] installer/hook compatibility حفظ شود، اما core Abolqasem workspace event-driven بماند.
...
- Legacy viewer asset references are relative, so the viewer can remain isolated from the Abolqasem workspace root.
- Existing legacy APIs and hook compatibility endpoints remain unchanged.

### Task 17.2: import یا نمایش legacy sessions بدون تغییر UX Abolqasem ✅
...
- [x] این بخش نباید sidebar اصلی Abolqasem را از project/chat model خارج کند.
...
- [readmodels.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/workspace/readmodels/readmodels.go)
- [types.ts](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/web-react/src/shared/types.ts)
- [useAbolqasemState.ts](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/web-react/src/client/app/useAbolqasemState.ts)
- [ChatRow.tsx](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/web-react/src/client/components/chat-ui/sidebar/ChatRow.tsx)
- Legacy sessions are overlaid into sidebar snapshots under project groups without being copied into the Abolqasem event store.
- Selecting a legacy chat imports its transcript into a read-only Abolqasem chat snapshot.
- Sidebar rows expose `readOnly` and `legacySessionKey`; the UI only marks read-only sessions.
...
- Abolqasem local HTTP serverهای مربوط به project را پیدا و مدیریت می‌کند.
...
- [x] project-specific browser preview behavior با Abolqasem یکی باشد.
...
- [x] UI Abolqasem بدون تغییر رفتاری کار کند.
...
- Quick actions persist in each project at `.abolqasem/quick-actions.json`, matching Abolqasem.
- Quick action normalization matches Abolqasem limits and duplicate handling.
...
- [x] response shape با Abolqasem برابر باشد.
...
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- Skills WebSocket commands now return Abolqasem-compatible shapes for search, install, uninstall, and installed snapshots.
- `skills.search` calls `skills.sh/api/search` with Abolqasem query and limit normalization.
- `skills.install` and `skills.uninstall` use the same `npx skills` command shape as Abolqasem.
...
- [x] attachment modes مطابق Abolqasem باشد: `metadata`, `bundle`.
- [x] themeهای export مطابق Abolqasem باشد: `light`, `dark`.
- [x] output با viewer/export Abolqasem سازگار باشد.
...
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- `chat.exportStandalone` now writes Abolqasem-compatible export directories under `.abolqasem/exports`.
...
- Share upload behavior and success/failure result shapes match Abolqasem.
...
- [x] backend result shape با Abolqasem برابر باشد.
...
- [useAbolqasemState.ts](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/web-react/src/client/app/useAbolqasemState.ts)
- [workspace_standalone_export.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_standalone_export.go)
- Share dialog wiring already matches Abolqasem and uses the `shareUrl` returned by `chat.exportStandalone`.
- Backend success and failure payloads now match the Abolqasem contract used by the dialog flow.
...
- [x] UpdateSnapshot با Abolqasem برابر باشد.
...
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- [workspace_composer.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_composer.go)
- `update.check` reads GitHub releases for the GoReleaser release stream while preserving the Abolqasem `UpdateSnapshot` shape.
- `update.install` schedules the Go binary update/restart path while preserving the Abolqasem `UpdateInstallResult` shape.
- Update snapshots are persisted in-memory and broadcast to `update` subscribers so the frontend reacts like Abolqasem.
...
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- `system.openExternal` now supports Abolqasem actions: `open_finder`, `open_terminal`, `open_editor`, `open_preview`, and `open_default`.
...
- [x] behavior فایل‌های `auth.ts` و `cli-runtime.ts` در Abolqasem بررسی و معادل Go آن مشخص شود.
- [x] اگر auth فقط برای feature خاص Abolqasem است، همان feature با همان UX port شود.
...
- [routes_test.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/routes_test.go)
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- Abolqasem `auth.ts` enables password auth only when CLI runtime passes a password. It uses an in-memory `abolqasem_session` cookie, same-origin Origin checks, and optional trusted proxy handling for secure cookies.
- Abolqasem `cli-runtime.ts` exposes `--password` together with host/share runtime flags. The current Go runtime is still local-only service/hook startup, so Task 24.1 keeps auth disabled instead of inventing remote multi-user auth.
- Disabled-auth parity is now explicit: `/auth/status` returns `{ enabled: false, authenticated: true }` and `/auth/logout` accepts `POST` with `{ ok: true }`, matching Abolqasem’s no-password UX shape.

### Task 24.2: Analytics parity ✅

Acceptance criteria:

- [x] analytics toggle و event naming با Abolqasem برابر باشد.
...
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- Abolqasem static analytics event/property names are mirrored in Go, including `analytics_enabled` and `analytics_disabled`.
- Abolqasem launch property derivation is mirrored as a reusable Go helper.
...
- [workspace_ws.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws.go)
- Machine display name now matches Abolqasem behavior: macOS prefers `scutil --get ComputerName`, other platforms use hostname with `.local`/`.lan` stripped, and empty names fall back to `This Machine`.
- Platform naming remains Abolqasem/Node-compatible by exposing Windows as `win32`.
- Abolqasem’s long-running CLI supervisor restarts child processes through exit codes `75`/`76`. The Go port already uses service/hook-aware `restart` and `update` commands plus detached scheduling, so no extra supervisor process is introduced for the current startup model.
...
- [x] auth جدید برای remote exposure خارج از parity scope است مگر Abolqasem همان را داشته باشد.
...
- Remote authentication is intentionally not added because Abolqasem parity is local-first and auth for remote exposure is outside current parity scope.
...
- [x] `system.ping` command envelope از مسیر `workspaceConnection.handle` به ack Abolqasem-compatible برسد.
...
- [workspace_ws_test.go](/home/h-mousavi/Projects/Hamed/codex-rtl-plugin/internal/server/workspace_ws_test.go)
- Backend command routing now has a direct smoke test for the Abolqasem envelope path, covering both a pure command and a state-mutating project/chat command sequence.
...
- Protocol parity with Abolqasem envelope.
...
- Existing Abolqasem frontend tests should be kept where practical.
...
1. Parity map کامل از Abolqasem.
2. Frontend copy با build موفق، بدون redesign.
3. i18n/RTL اضافه شود، بدون تغییر visual identity.
4. Go WebSocket protocol دقیقاً با envelope Abolqasem.
5. EventStore و ReadModels مطابق Abolqasem.
6. Project/chat CRUD مطابق Abolqasem.
...
- share/export کامل.
- analytics behavior مطابق Abolqasem.
- skills کامل.
- update install کامل.
- BrowserPanel کامل.

Gemini از این لیست حذف شد چون در Abolqasem provider اصلی نیست. Gemini فقط برای legacy compatibility پروژه فعلی می‌تواند جداگانه حفظ شود، نه در Abolqasem parity workspace.

اولین vertical slice باید این را ثابت کند:

```text
React Abolqasem UI copied with minimal change
...
### Risk: Frontend copy too tightly coupled to Abolqasem protocol

Mitigation:

- این ریسک عمداً پذیرفته می‌شود چون هدف copy کردن Abolqasem است.
- به جای تغییر frontend، Go backend باید Abolqasem protocol را پیاده کند.
...
## Definition Of Done For Final Abolqasem Parity

Final parity زمانی کامل است که:

- ظاهر اصلی Abolqasem حفظ شده باشد.
- sidebar، local projects، chat page، settings، right sidebar، terminal و browser panel مطابق Abolqasem کار کنند.
- تمام commandهای `ClientCommand` در Abolqasem یا پیاده شده باشند یا با دلیل explicit و documented حذف شده باشند.
- تمام topicهای `SubscriptionTopic` در Abolqasem پشتیبانی شوند.
- تمام snapshotهای `ServerSnapshot` در Abolqasem پشتیبانی شوند.
- `claude` و `codex` providerها مطابق Abolqasem کار کنند.
- model/reasoning/context/fast/plan controls مطابق Abolqasem باشند.
- queue، steer، dequeue، cancel و stop draining مطابق Abolqasem باشند.
- tool approval و AskUserQuestion/ExitPlanMode مطابق Abolqasem باشند.
- Git/Diff/GitHub workflows مطابق Abolqasem باشند.
- terminal مطابق Abolqasem باشد.
- BrowserPanel/local HTTP servers مطابق Abolqasem باشد.
- quick actions مطابق Abolqasem باشد.
- skills مطابق Abolqasem باشد.
- settings/keybindings/LLM provider مطابق Abolqasem باشد.
- standalone export/share مطابق Abolqasem باشد.
- update/check/install مطابق Abolqasem UI contract باشد.
