# Abolqasem Parity Map

This file tracks what must be copied, ported, deferred, or excluded while porting Abolqasem to `ai-agent-manager` with a Go backend.
...
- Copy Abolqasem frontend structure and behavior first.
- Do not redesign UX or invent product flows.
- Preserve Abolqasem WebSocket protocol shapes where possible.
- Port backend behavior to Go.
- Add only required changes for Go backend, branding, Persian i18n, and RTL.

## Server Modules

| Abolqasem module | Go target | Status | Notes |
| --- | --- | --- | --- |
...
| `provider-catalog.ts` | `internal/providers/catalog` | `[done]` | Abolqasem `claude` and `codex` server catalog ported. |
| `event-store.ts` | `internal/workspace/eventstore` | `[in-progress]` | JSONL append/replay implemented; snapshot compaction remains. |
| `events.ts` | `internal/workspace/events` | `[done]` | Abolqasem v2 top-level event shape preserved. |
| `read-models.ts` | `internal/workspace/readmodels` | `[done]` | Sidebar, chat, and local project read models implemented. |
| `ws-router.ts` | `internal/workspace/ws` | `[in-progress]` | Abolqasem envelope-compatible WS endpoint and initial snapshots implemented. |
...
| `auth.ts` | `internal/auth` | `[todo]` | Port only Abolqasem behavior, no new auth product. |
| `analytics.ts` | `internal/analytics` | `[todo]` | Keep Abolqasem-compatible setting/shape. |
| `machine-name.ts` | `internal/machine` | `[todo]` | Machine label where needed. |
| `paths.ts` | `internal/paths` | `[todo]` | Abolqasem data path equivalents in Go. |
...
| Abolqasem module | Go/TS target | Status | Notes |
| --- | --- | --- | --- |
...
| `analytics.ts` | frontend shared + Go analytics | `[todo]` | Keep Abolqasem event shapes if enabled. |
...
- `[done]` Abolqasem frontend source copied to `web-react/src/client`, `web-react/src/shared`, and `web-react/src/export-viewer`.
- `[done]` Abolqasem public assets copied to `web-react/public`.
- `[done]` Abolqasem frontend build config copied and adjusted for npm/Vite.
- `[done]` i18n/RTL foundation added.
- `[in-progress]` Go WebSocket integration has initial Abolqasem-compatible endpoint and snapshots.

| Abolqasem path | Status | Notes |
| --- | --- | --- |
...
| `app/AbolqasemSidebar.tsx` | `[todo]` | Copy, add i18n/RTL. |
| `app/AbolqasemTranscript.tsx` | `[todo]` | Copy. |
...
| `app/useAbolqasemState.ts` | `[todo]` | Copy, adapt backend calls only where unavoidable. |
...
| Add locale setting | `[done]` | Integrated into Abolqasem settings UI. |
| Apply `dir=rtl` for Persian | `[done]` | Document direction follows locale. |
| Keep code/diff/terminal/path LTR | `[in-progress]` | Existing Abolqasem components preserve LTR-oriented rendering; full audit remains. |
...
- Gemini remains legacy-viewer-only unless Abolqasem upstream adds Gemini provider support.
- No new remote auth or multi-user product flow is part of Abolqasem parity.
