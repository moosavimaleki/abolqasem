...
func TestLocalServerAddressBindsLoopbackOnly(t *testing.T) {
	if got := localServerAddress(9090); got != "127.0.0.1:9090" {
...
func TestWorkspaceWSOriginAllowedOnlyForLoopbackHTTP(t *testing.T) {
	tests := []struct {
		...
		{name: "localhost", origin: "http://localhost:9090", allowed: true},
		{name: "loopback", origin: "http://127.0.0.1:9090", allowed: true},
		{name: "remote host", origin: "http://example.com:9090", allowed: false},
		{name: "localhost subdomain", origin: "http://localhost.example.com:9090", allowed: false},
		{name: "credentials", origin: "http://localhost:9090@evil.example", allowed: false},
		{name: "https remote exposure", origin: "https://localhost:9090", allowed: false},
		{name: "pathful origin", origin: "http://localhost:9090/app", allowed: false},
	}
